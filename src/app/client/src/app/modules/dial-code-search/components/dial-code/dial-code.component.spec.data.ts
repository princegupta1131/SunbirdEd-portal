export const Response = {
    // tslint:disable-next-line:max-line-length
    successData: {
        'id': 'api.v3.read',
        'ver': '1.0',
        'ts': '2020-04-18T04:13:46.475Z',
        'params': {
            'resmsgid': 'ff9473b0-812a-11ea-833f-1565f5d5bc45',
            'msgid': 'ff902df0-812a-11ea-833f-1565f5d5bc45',
            'status': 'successful',
            'err': null,
            'errmsg': null
        },
        'responseCode': 'OK',
        'result': {
            'content': {
                'ownershipType': [
                    'createdBy'
                ],
                'copyright': '345 org, Sunbird QA Tenant, SunbirdQA 3',
                'keywords': [
                    'test',
                    'nadiya'
                ],
                'year': '2005',
                'subject': [
                    'Mathematics'
                ],
                'channel': '0124511394914140160',
                // tslint:disable-next-line: max-line-length
                'downloadUrl': 'https://ntpstagingall.blob.core.windows.net/ntp-content-staging/ecar_files/do_212925261140451328114/rf-book-3_1577791108524_do_212925261140451328114_1.0_spine.ecar',
                'organisation': [
                    '345 org',
                    'Sunbird QA Tenant',
                    'SunbirdQA 3'
                ],
                'language': [
                    'English'
                ],
                'mimeType': 'application/vnd.ekstep.content-collection',
                'variants': {
                    'online': {
                        // tslint:disable-next-line: max-line-length
                        'ecarUrl': 'https://ntpstagingall.blob.core.windows.net/ntp-content-staging/ecar_files/do_212925261140451328114/rf-book-3_1577791108897_do_212925261140451328114_1.0_online.ecar',
                        'size': 10604
                    },
                    'spine': {
                        // tslint:disable-next-line: max-line-length
                        'ecarUrl': 'https://ntpstagingall.blob.core.windows.net/ntp-content-staging/ecar_files/do_212925261140451328114/rf-book-3_1577791108524_do_212925261140451328114_1.0_spine.ecar',
                        'size': 78911
                    }
                },
                'leafNodes': [
                    'do_2125678785313259521740',
                    'do_2125734955775508481459',
                    'do_212915144844754944118',
                    'do_212910965326520320112',
                    'do_21291449719969382411',
                    'do_2125678797343047681742'
                ],
                'objectType': 'Content',
                'gradeLevel': [
                    'Class 9'
                ],
                // tslint:disable-next-line: max-line-length
                'appIcon': 'https://ntpstagingall.blob.core.windows.net/ntp-content-staging/content/do_212925261140451328114/artifact/0fef6edb-7cd4-4388-9237-36a5331ed00e_1571034457477.thumb.jpg',
                'children': [
                    {
                        'identifier': 'do_21291449719969382411',
                        'name': '16 Dec',
                        'description': 'Enter description for Resource',
                        'objectType': 'Content',
                        'relation': 'hasSequenceMember',
                        'status': 'Live'
                    },
                    {
                        'identifier': 'do_212915144844754944118',
                        'name': '17_Dece_Resource',
                        'description': 'Enter description for Resource',
                        'objectType': 'Content',
                        'relation': 'hasSequenceMember',
                        'status': 'Live'
                    },
                    {
                        'identifier': 'do_2125678797343047681742',
                        'name': 'PDF content',
                        'description': 'by um',
                        'objectType': 'Content',
                        'relation': 'hasSequenceMember',
                        'status': 'Live'
                    },
                    {
                        'identifier': 'do_2125734955775508481459',
                        'name': 'Resource_test55',
                        'description': 'Untitled Collection',
                        'objectType': 'Content',
                        'relation': 'hasSequenceMember',
                        'status': 'Live'
                    },
                    {
                        'identifier': 'do_2125678785313259521740',
                        'name': 'EPUB content',
                        'description': 'by um',
                        'objectType': 'Content',
                        'relation': 'hasSequenceMember',
                        'status': 'Live'
                    },
                    {
                        'identifier': 'do_212910965326520320112',
                        'name': 'Nadiya_L',
                        'description': 'Sample',
                        'objectType': 'Content',
                        'relation': 'hasSequenceMember',
                        'status': 'Live'
                    }
                ],
                'appId': 'staging.diksha.portal',
                'contentEncoding': 'gzip',
                'lockKey': 'c55a6cd8-4a79-420c-8241-7927381853d3',
                'totalCompressedSize': 2486409,
                // tslint:disable-next-line: max-line-length
                'mimeTypesCount': '{"application/pdf":1,"application/epub":1,"application/vnd.ekstep.content-collection":4,"application/vnd.ekstep.ecml-archive":4}',
                'sYS_INTERNAL_LAST_UPDATED_ON': '2019-12-31T11:18:29.123+0000',
                'contentType': 'TextBook',
                'identifier': 'do_212925261140451328114',
                'lastUpdatedBy': '25827334-616f-41fc-bc13-b4a1b0b4d7f5',
                'audience': [
                    'Learner'
                ],
                // tslint:disable-next-line: max-line-length
                'toc_url': 'https://ntpstagingall.blob.core.windows.net/ntp-content-staging/content/do_212925261140451328114/artifact/do_212925261140451328114_toc.json',
                'visibility': 'Default',
                'contentTypesCount': '{"TextBookUnit":4,"Resource":6}',
                'consumerId': '56ff6913-abcc-4a88-b247-c976e47cbfb4',
                'childNodes': [
                    'do_2129252615893401601110',
                    'do_2125678785313259521740',
                    'do_2125734955775508481459',
                    'do_2129252615893401601111',
                    'do_212915144844754944118',
                    'do_2129252615893319681109',
                    'do_212910965326520320112',
                    'do_21291449719969382411',
                    'do_2129252615893565441112',
                    'do_2125678797343047681742'
                ],
                'mediaType': 'content',
                'osId': 'org.ekstep.quiz.app',
                'languageCode': [
                    'en'
                ],
                'lastPublishedBy': '00c3affc-6988-4b38-9d61-c7bd23ac6cec',
                'version': 2,
                'rejectReasons': [
                    'Incorrect Board, Grade, Subject or Medium'
                ],
                'license': 'CC BY-NC-SA 4.0',
                'prevState': 'Review',
                'size': 78911,
                'lastPublishedOn': '2019-12-31T11:18:28.403+0000',
                'name': 'RF Book 3',
                'rejectComment': 'nolt correct',
                'publisher': 'Juthika',
                'status': 'Live',
                'code': 'org.sunbird.zPXGMQ',
                'description': 'Enter description for TextBook',
                'medium': [
                    'Hindi'
                ],
                // tslint:disable-next-line: max-line-length
                'posterImage': 'https://ntpstagingall.blob.core.windows.net/ntp-content-staging/content/do_2128699142366658561506/artifact/0fef6edb-7cd4-4388-9237-36a5331ed00e_1571034457477.jpg',
                'idealScreenSize': 'normal',
                'createdOn': '2019-12-31T11:11:06.567+0000',
                'copyrightYear': 2019,
                'contentDisposition': 'inline',
                'lastUpdatedOn': '2019-12-31T11:18:28.124+0000',
                'dialcodeRequired': 'No',
                'lastStatusChangedOn': '2019-12-31T11:18:29.112+0000',
                'createdFor': [
                    '0124511394914140160',
                    '01245114878763827244',
                    '01231711180382208027'
                ],
                'creator': 'Book Creator',
                'os': [
                    'All'
                ],
                'pkgVersion': 1,
                'versionKey': '1577791108124',
                'idealScreenDensity': 'hdpi',
                'framework': 'rj_k-12_1',
                'dialcodes': [
                    'T4S6T3'
                ],
                'depth': 0,
                's3Key': 'ecar_files/do_212925261140451328114/rf-book-3_1577791108524_do_212925261140451328114_1.0_spine.ecar',
                'lastSubmittedOn': '2019-12-31T11:17:30.705+0000',
                'createdBy': '25827334-616f-41fc-bc13-b4a1b0b4d7f5',
                'compatibilityLevel': 1,
                'leafNodesCount': 6,
                'board': 'State (Rajasthan)',
                'resourceType': 'Book'
            }
        }
    },
    noData: {
        'id': 'api.v1.search', 'ver': '1.0', 'ts': '2018-05-08T05:58:54.352Z', 'params': {
            'resmsgid': 'e3a9d900-5284-11e8-a042-f5d5c4eb1686', 'msgid': 'e3a39770-5284-11e8-b0c4-bb12c891d4ac', 'status': 'successful',
            'err': null, 'errmsg': null
        }, 'responseCode': 'OK', 'result': { 'count': 0, 'content': [] }
    },
    resultdata: [{
        'ribbon': {
            'left': {
                'class': 'ui blue left ribbon label',
                'name': undefined
            },
            'right': {
                'class': 'ui black right ribbon label',
                'name': 'LessonPlan'
            }
        },
        'description': undefined,
        'image': `https://ekstep-public-dev.s3-ap-south-1.amazonaws.com/content/do_112485067337392128145/artifact/2_1466416664900.thumb.jpg`,
        'action': {
            'onImage': { eventName: 'onImage' }
        },
        'metaData': {
            'contentType': 'LessonPlan',
            'framework': 'NCF',
            'identifier': 'do_112485067337392128145',
            'mimeType': 'application/vnd.ekstep.content-collection'
        },
        'name': 'lp test 4',
        'rating': '0'
    }],

    event: {
        action: {
            eventName: 'onImage'
        },
        data: {
            'ribbon': {
                'left': {
                    'class': 'ui blue left ribbon label',
                    'name': undefined
                },
                'right': {
                    'class': 'ui black right ribbon label',
                    'name': 'LessonPlan'
                }
            },
            'description': undefined,
            'image': `https://ekstep-public-dev.s3-ap-south-1.amazonaws.com/content/do_112485067337392128145/artifact/2_1466416664900.thumb.jpg`,
            'action': {
                'onImage': { eventName: 'onImage' }
            },
            'metaData': {
                'contentType': 'LessonPlan',
                'framework': 'NCF',
                'identifier': 'do_112485067337392128145',
                'mimeType': 'application/vnd.ekstep.content-collection'
            },
            'name': 'lp test 4',
            'rating': '0'
        }
    },
    download_event:
    {
        'action': 'download',
        'data': {
            'action':
            {
                'onImage': { 'eventName': 'onImage' }
            },
            'addedToLibrary': true,
            'completionPercentage': 0,
            'contentType': 'Resource',
            'description': 'Math-Magic_7_Jugs and Mugs_Bunny and Banno celebrate their Wedding Anniversary_Introduction to volume',
            'gradeLevel': 'Class 4',
            // tslint:disable-next-line:max-line-length
            'image': 'https://ekstep-public-prod.s3-ap-south-1.amazonaws.com/content/do_312579855868370944110877/artifact/1n4jfaogvexvuuff6knjagpzmavlvdxk2.thumb.png',
            'medium': 'English',
            // tslint:disable-next-line:max-line-length
            'metaData': { 'identifier': 'do_312579855868370944110877', 'mimeType': 'video/x-youtube', 'framework': 'ekstep_sample-framework_k-12', 'contentType': 'Resource' },
            'name': 'Jugs and Mugs_Bunny and Banno celebrate their Wedding Anniversary_2',
            'orgDetails': {},
            'rating': 3,
            'ribbon': {
                'left': {
                    'class': 'ui circular label  card-badges-image',
                    'image': 'https://ntpproduction.blob.core.windows.net/badgr/uploads/badges/739851bf8ecd6203aa5dd2d9de155f31.png'
                },
                'right': {
                    'class': 'ui black right ribbon label',
                    'name': 'Learn'
                },
            },
            'subTopic': '',
            'subject': 'Mathematics',
            'topic': 'Volumes and Capacity'
        },
        'section': 'Featured Content'
    },
    textBookSearchResult:
        [
            {
                'display': '{"name":{"en":"Linked Content"}}',
                'alt': null,
                'count': 1,
                'description': null,
                'index': 1,
                'sectionDataType': 'content',
                'facets': [
                    {
                        'values': [
                            {
                                'name': 'mathematics',
                                'count': 1
                            }
                        ],
                        'name': 'subject'
                    },
                    {
                        'values': [],
                        'name': 'grade'
                    },
                    {
                        'values': [],
                        'name': 'domain'
                    },
                    {
                        'values': [
                            {
                                'name': 'english',
                                'count': 1
                            }
                        ],
                        'name': 'language'
                    },
                    {
                        'values': [
                            {
                                'name': 'hindi',
                                'count': 1
                            }
                        ],
                        'name': 'medium'
                    },
                    {
                        'values': [
                            {
                                'name': 'textbook',
                                'count': 1
                            }
                        ],
                        'name': 'contentType'
                    }
                ],
                'collectionsCount': 0,
                'imgUrl': null,
                'resmsgId': 'edf22640-80bf-11ea-833f-1565f5d5bc45',
                'collections': [],
                'contents': [
                    {
                        'ownershipType': [
                            'createdBy'
                        ],
                        'copyright': '345 org, Sunbird QA Tenant, SunbirdQA 3',
                        'keywords': [
                            'test',
                            'nadiya'
                        ],
                        'year': '2005',
                        'subject': 'Mathematics',
                        'channel': '0124511394914140160',
                        // tslint:disable-next-line: max-line-length
                        'downloadUrl': 'https://ntpstagingall.blob.core.windows.net/ntp-content-staging/ecar_files/do_212925261140451328114/rf-book-3_1577791108524_do_212925261140451328114_1.0_spine.ecar',
                        'organisation': [
                            '345 org',
                            'Sunbird QA Tenant',
                            'SunbirdQA 3'
                        ],
                        'language': [
                            'English'
                        ],
                        'mimeType': 'application/vnd.ekstep.content-collection',
                        'variants': {
                            'online': {
                                // tslint:disable-next-line: max-line-length
                                'ecarUrl': 'https://ntpstagingall.blob.core.windows.net/ntp-content-staging/ecar_files/do_212925261140451328114/rf-book-3_1577791108897_do_212925261140451328114_1.0_online.ecar',
                                'size': 10604
                            },
                            'spine': {
                                // tslint:disable-next-line: max-line-length
                                'ecarUrl': 'https://ntpstagingall.blob.core.windows.net/ntp-content-staging/ecar_files/do_212925261140451328114/rf-book-3_1577791108524_do_212925261140451328114_1.0_spine.ecar',
                                'size': 78911
                            }
                        },
                        'leafNodes': [
                            'do_2125678785313259521740',
                            'do_2125734955775508481459',
                            'do_212915144844754944118',
                            'do_212910965326520320112',
                            'do_21291449719969382411',
                            'do_2125678797343047681742'
                        ],
                        'objectType': 'Content',
                        'gradeLevel': [
                            'Class 9'
                        ],
                        // tslint:disable-next-line: max-line-length
                        'appIcon': 'https://ntpstagingall.blob.core.windows.net/ntp-content-staging/content/do_212925261140451328114/artifact/0fef6edb-7cd4-4388-9237-36a5331ed00e_1571034457477.thumb.jpg',
                        'children': [
                            'do_212910965326520320112',
                            'do_21291449719969382411',
                            'do_212915144844754944118',
                            'do_2125734955775508481459',
                            'do_2125678785313259521740',
                            'do_2125678797343047681742'
                        ],
                        'appId': 'staging.diksha.portal',
                        'contentEncoding': 'gzip',
                        'lockKey': 'c55a6cd8-4a79-420c-8241-7927381853d3',
                        // tslint:disable-next-line: max-line-length
                        'mimeTypesCount': '{"application/pdf":1,"application/epub":1,"application/vnd.ekstep.content-collection":4,"application/vnd.ekstep.ecml-archive":4}',
                        'totalCompressedSize': 2486409,
                        'contentType': 'TextBook',
                        'identifier': 'do_212925261140451328114',
                        'lastUpdatedBy': '25827334-616f-41fc-bc13-b4a1b0b4d7f5',
                        'audience': [
                            'Learner'
                        ],
                        'visibility': 'Default',
                        // tslint:disable-next-line: max-line-length
                        'toc_url': 'https://ntpstagingall.blob.core.windows.net/ntp-content-staging/content/do_212925261140451328114/artifact/do_212925261140451328114_toc.json',
                        'contentTypesCount': '{"TextBookUnit":4,"Resource":6}',
                        'consumerId': '56ff6913-abcc-4a88-b247-c976e47cbfb4',
                        'childNodes': [
                            'do_2129252615893401601110',
                            'do_2125678785313259521740',
                            'do_2125734955775508481459',
                            'do_2129252615893401601111',
                            'do_212915144844754944118',
                            'do_2129252615893319681109',
                            'do_212910965326520320112',
                            'do_21291449719969382411',
                            'do_2129252615893565441112',
                            'do_2125678797343047681742'
                        ],
                        'mediaType': 'content',
                        'osId': 'org.ekstep.quiz.app',
                        'graph_id': 'domain',
                        'nodeType': 'DATA_NODE',
                        'lastPublishedBy': '00c3affc-6988-4b38-9d61-c7bd23ac6cec',
                        'version': 2,
                        'rejectReasons': [
                            'Incorrect Board, Grade, Subject or Medium'
                        ],
                        'license': 'CC BY-NC-SA 4.0',
                        'prevState': 'Review',
                        'size': 78911,
                        'lastPublishedOn': '2019-12-31T11:18:28.403+0000',
                        'IL_FUNC_OBJECT_TYPE': 'Content',
                        'name': 'RF Book 3',
                        'rejectComment': 'nolt correct',
                        'publisher': 'Juthika',
                        'status': 'Live',
                        'code': 'org.sunbird.zPXGMQ',
                        'description': 'Enter description for TextBook',
                        'medium': 'Hindi',
                        'idealScreenSize': 'normal',
                        // tslint:disable-next-line: max-line-length
                        'posterImage': 'https://ntpstagingall.blob.core.windows.net/ntp-content-staging/content/do_2128699142366658561506/artifact/0fef6edb-7cd4-4388-9237-36a5331ed00e_1571034457477.jpg',
                        'createdOn': '2019-12-31T11:11:06.567+0000',
                        'copyrightYear': 2019,
                        'contentDisposition': 'inline',
                        'lastUpdatedOn': '2019-12-31T11:18:28.124+0000',
                        'SYS_INTERNAL_LAST_UPDATED_ON': '2019-12-31T11:18:29.123+0000',
                        'dialcodeRequired': 'No',
                        'creator': 'Book Creator',
                        'createdFor': [
                            '0124511394914140160',
                            '01245114878763827244',
                            '01231711180382208027'
                        ],
                        'lastStatusChangedOn': '2019-12-31T11:18:29.112+0000',
                        'IL_SYS_NODE_TYPE': 'DATA_NODE',
                        'os': [
                            'All'
                        ],
                        'pkgVersion': 1,
                        'versionKey': '1577791108124',
                        'idealScreenDensity': 'hdpi',
                        'framework': 'rj_k-12_1',
                        'depth': 0,
                        'dialcodes': [
                            'T4S6T3'
                        ],
                        's3Key': 'ecar_files/do_212925261140451328114/rf-book-3_1577791108524_do_212925261140451328114_1.0_spine.ecar',
                        'lastSubmittedOn': '2019-12-31T11:17:30.705+0000',
                        'createdBy': '25827334-616f-41fc-bc13-b4a1b0b4d7f5',
                        'compatibilityLevel': 1,
                        'leafNodesCount': 6,
                        'IL_UNIQUE_ID': 'do_212925261140451328114',
                        'board': 'State (Rajasthan)',
                        'resourceType': 'Book',
                        'node_id': 520821
                    }
                ],
                // tslint:disable-next-line: max-line-length
                'searchQuery': '{"request":{"facets":["language","grade","domain","contentType","subject","medium"],"filters":{"contentType":["TextBook","TextBookUnit"],"objectType":["Content"],"status":["Live"],"compatibilityLevel":{"max":4,"min":1}},"mode":"collection"},"limit":10,"sort_by":{"lastUpdatedOn":"desc"}}',
                'name': 'Linked Content',
                'id': '0126541330342952961',
                'dynamicFilters': null,
                'dataSource': null,
                'apiId': 'api.content.search',
                'group': 1
            }
        ],
    'inview': [
        {
            'id': 0,
            'data': {
                'name': 'Aman15thMay Book',
                'image': `https://ekstep-public-dev.s3-ap-south-1.amazonaws.com/content
                  /do_1125041330714050561226/artifact/4272f16cf3fd329b18dd116315601ad0_1476257845556.thumb.jpeg`,
                'description': 'Untitled Collection sadasd asd asd as',
                'action': {
                    'right': {
                        'class': 'trash large icon',
                        'eventName': 'delete',
                        'displayType': 'icon'
                    },
                    'onImage': {
                        'eventName': 'onImage'
                    }
                },
                'ribbon': {
                    'right': {
                        'name': 'TextBook',
                        'class': 'ui black right ribbon label'
                    }
                },
                'metaData': {
                    'identifier': 'do_1125041330714050561226',
                    'mimeType': 'application / vnd.ekstep.content - collection',
                    'framework': 'NCF',
                    'contentType': 'TextBook'
                }
            }
        },
        {
            'id': 1,
            'data': {
                'name': 'Untitled Collection',
                'description': 'Untitled Collection',
                'action': {
                    'right': {
                        'class': 'trash large icon',
                        'eventName': 'delete',
                        'displayType': 'icon'
                    },
                    'onImage': {
                        'eventName': 'onImage'
                    }
                },
                'ribbon': {
                    'right': {
                        'name': 'Resource',
                        'class': 'ui black right ribbon label'
                    }
                },
                'metaData': {
                    'identifier': 'do_112514006444826624126',
                    'mimeType': 'application / vnd.ekstep.ecml - archive',
                    'framework': 'NCF',
                    'contentType': 'Resource'
                }
            }
        }
    ]
};
